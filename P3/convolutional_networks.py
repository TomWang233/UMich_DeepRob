...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                for f in range(F):
                    out[:, f, i, j] = torch.sum(x_mask * w[f, :, :, :], (1, 2, 3)) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        W_out = int(1 + (W + 2 * pad - WW) / stride)
        x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad), 'constant', 0)
        dx_pad = torch.zeros_like(x_pad, dtype=x.dtype)
        dw = torch.zeros_like(w, dtype=x.dtype)
        db = torch.zeros_like(b, dtype=x.dtype)
        for i in range(H_out):
            for j in range(W_out):
                x_mask = x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
                for f in range(F):
                    dw[f, :, :, :] += torch.sum(dout[:, f, i, j].reshape(-1, 1, 1, 1) * x_mask, 0)
                    db[f] += torch.sum(dout[:, f, i, j])
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                out[:, :, i, j] = torch.max(x_mask.reshape(N, C, -1), 2)[0]
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(H_out):
            for j in range(W_out):
                x_mask = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                x_mask_max = torch.max(x_mask.reshape(N, C, -1), 2)[0].reshape(N, C, 1, 1)
                dx_mask = (x_mask == x_mask_max) * 1
                dx[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] += dx_mask * dout[:, :, i, j].reshape(N, C, 1, 1)
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        for k, v in self.params.items():
            self.params[k] = v.to(device=device, dtype=dtype)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(len(num_filters)):
            if weight_scale == 'kaiming':
                weight = kaiming_initializer(C, num_filters[i], 3, device=device, dtype=self.dtype)
            else:
                weight = weight_scale
            self.params['W'+str(i+1)] = weight * torch.randn(num_filters[i], C, 3, 3, device=device)
            self.params['b'+str(i+1)] = torch.zeros(num_filters[i], device=device)
            C = num_filters[i]
            if batchnorm:
                self.params['gamma'+str(i+1)] = torch.ones(num_filters[i], device=device)
                self.params['beta'+str(i+1)] = torch.zeros(num_filters[i], device=device)
            if i in max_pools:
                H = (H + 1) // 2
                W = (W + 1) // 2
        if weight_scale == 'kaiming':
            weight = kaiming_initializer(C*H*W, num_classes, device=device, dtype=self.dtype)
        else:
            weight = weight_scale
        self.params['W'+str(len(num_filters)+1)] = weight * torch.randn(C*H*W, num_classes, device=device)
        self.params['b'+str(len(num_filters)+1)] = torch.zeros(num_classes, device=device)
        for k, v in self.params.items():
            self.params[k] = v.to(dtype=dtype)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            W = self.params['W'+str(i+1)]
            reg_sum += torch.sum(W * W)
        loss += self.reg * reg_sum
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers - 1:
                dout, grads['W'+str(i+1)], grads['b'+str(i+1)] = Linear.backward(dout, caches[i])
                W = self.params['W'+str(i+1)]
                grads['W'+str(i+1)] += 2 * self.reg * W
            else:
                W = self.params['W'+str(i+1)]
                b = self.params['b'+str(i+1)]
                do_pool = i in self.max_pools
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn(Din, Dout, device=device, dtype=dtype) * gain / Din
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn(Dout, Din, K, K, device=device, dtype=dtype) * gain / (Din * K * K)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            xmu = x - sample_mean
            sq = xmu ** 2
            sample_var = torch.mean(sq, dim=0)
            sqrtvar = torch.sqrt(sample_var + eps)
            ivar = 1. / sqrtvar
            xhat = xmu * ivar
            gammax = gamma * xhat
            out = gammax + beta
