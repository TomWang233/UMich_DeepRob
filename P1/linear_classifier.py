...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        diff = scores - torch.max(scores)
        loss += torch.log(torch.sum(torch.exp(diff))) - diff[y[i]]
        for j in range(num_classes):
            temp = torch.exp(diff[j]) / torch.sum(torch.exp(diff))
            if j == y[i]:
                dW[:, j] += (-1 + temp) * X[i]
            else:
                dW[:, j] += temp * X[i]
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = X @ W
    diffs = scores - torch.max(scores, dim=1).values.unsqueeze(-1)
    temp = torch.exp(diffs) / torch.sum(torch.exp(diffs), dim=1).unsqueeze(-1)
    loss = -torch.sum(torch.log(temp[range(num_train), y]))
    loss = loss / num_train + reg * torch.sum(W * W)
