...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test): 
            temp = (x_train[i] - x_test[j]).pow(2)
            dists[i,j] = torch.sum(temp)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = torch.flatten(x_test, start_dim=1)
    for i in range(num_train):
        temp = (x_test - x_train[i]).pow(2)
        dists[i] = torch.sum(temp, 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = torch.flatten(x_train, start_dim=1)
    x_test = torch.flatten(x_test, start_dim=1)
    x_train_squared = torch.sum(x_train.pow(2), 1).reshape(-1,1)
    #print('x_train shape: ', x_train_squared.shape)
    x_test_squared = torch.sum(x_test.pow(2), 1).reshape(1,-1)
    #print('x_test shape: ', x_test_squared.shape)
    middle = 2*(x_train.mm(x_test.t()))
    #print('middle shape: ', middle.shape)
    dists = x_train_squared - middle + x_test_squared
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        output, counts = torch.unique(selected_y_train, return_counts=True)
        #print(output)
        #print(counts)
        most_appearence = output[counts.argmax()]
        #print(most_appearence)
        y_pred[i] = most_appearence
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        dist_matrix = compute_distances_no_loops(self.x_train, x_test)
        y_test_pred = predict_labels(dist_matrix, self.y_train)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            selected_y_train = []
            for j in index:
                selected_x_train.append(x_train_folds[j])
                selected_y_train.append(y_train_folds[j])
            cross_x_train = torch.cat(selected_x_train)
            #print(cross_x_train.shape)
            cross_y_train = torch.cat(selected_y_train)
            #print(cross_y_train.shape)
            cross_x_test = x_train_folds[i]
            #print(cross_x_test.shape)
            cross_y_test = y_train_folds[i]
            #print(cross_y_test.shape)
            knn = KnnClassifier(cross_x_train, cross_y_train)
            k_to_accuracies[k].append(knn.check_accuracy(cross_x_test, cross_y_test, k=k, quiet=True))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    means = []
    my_dict = sorted(k_to_accuracies.items())
    #print(my_dict)
    for k, accs in my_dict:
        means.append(sum(accs) / len(accs))
    best_k = my_dict[torch.tensor(means).argmax()][0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
