...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    #print(b2.shape)
    #print(X.mm(W1).shape)
    first_operation = X.mm(W1)+b1.reshape(1, -1)
    #print(first_operation.shape)
    hidden = torch.max(torch.tensor([0],dtype=X.dtype, device=X.device), 
                      first_operation.to(X.dtype))
    #print(hidden)
    #print(hidden.shape)         
...
def nn_forward_backward(
    ...
    #print(exp_scores)
    #print(torch.sum(exp_scores,1).reshape(-1,1))
    #print(exp_scores / torch.sum(exp_scores,1).reshape(-1,1))
    normalized_scores = exp_scores / (torch.sum(exp_scores,1).reshape(-1,1))
    #print(normalized_scores)
    #print(y)
    correct_label_scores = normalized_scores[range(N), y]
    #print(correct_label_scores)
    log_scores = -1 * torch.log(correct_label_scores)
    #print(log_scores)
    L2_reg = reg*(torch.sum(W1**2) + torch.sum(W2**2))
    #print(L2_reg)
    loss = torch.sum(log_scores) / N + L2_reg
    #print(loss)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    first_operation = X.mm(params['W1'])+params['b1'].reshape(1, -1)
    hidden = torch.max(torch.tensor([0],dtype=X.dtype, device=X.device), 
                      first_operation.to(X.dtype))
    scores = hidden.mm(params['W2'])+params['b2'].reshape(1, -1)
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hss, rss, lrd = get_param_set_fn()
    for lr in lrs:
        for lr_decay in lrd:
            for reg in rss:
                for hs in hss:
                  ...
                  results = net.train(data_dict['X_train'], data_dict['y_train'], 
                                      data_dict['X_val'], data_dict['y_val'],
                                      num_iters=3000, batch_size=1000,
                                      learning_rate=lr, learning_rate_decay=lr_decay,
                                      reg=reg, verbose=False)
                  y_train_pred = net.predict(data_dict['X_train'])
