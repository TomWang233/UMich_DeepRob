...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        temp = x.clone().detach()
        dx = dout*torch.tensor(temp>0, dtype=dout.dtype, device=dout.device)
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
        return dx
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        middle, LR_cache = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, L_cache = Linear.forward(middle, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        reg_sum = self.reg*(torch.sum(self.params['W1']**2)
                            +torch.sum(self.params['W2']**2))
        loss = loss + reg_sum
        dx2, dw2, db2 = Linear.backward(dscores, L_cache)
        
        _, dw1, db1 = Linear_ReLU.backward(dx2, LR_cache)
        grads['W2'] = dw2 + 2*self.params['W2']*self.reg
        grads['b2'] = db2
        grads['W1'] = dw1 + 2*self.params['W1']*self.reg
        grads['b1'] = db1
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Replace "pass" statement with your code
        parameters = []
        parameters.append(input_dim)
        for i in range(len(hidden_dims)):
          parameters.append(hidden_dims[i])
        parameters.append(num_classes)
        for i in range(self.num_layers):
          self.params[f'W{i+1}'] = torch.randn((parameters[i],parameters[i+1]), dtype = dtype, 
                                                device = device)*weight_scale
          self.params[f'b{i+1}'] = torch.zeros(parameters[i+1], dtype = dtype, device = device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          if i == (self.num_layers-1):
            scores, temp_cache = Linear.forward(scores, self.params[f'W{i+1}'],
                                                    self.params[f'b{i+1}'])
          else:
            scores, temp_cache = Linear_ReLU.forward(scores, self.params[f'W{i+1}'],
                                                    self.params[f'b{i+1}'])
          cache_list.append(temp_cache)
          if i != (self.num_layers-1) and self.use_dropout:
            scores, temp_cache = Dropout.forward(scores, self.dropout_param)
            cache_list.append(temp_cache)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        reg_sum = 0
        for i in range(self.num_layers):
          reg_sum = reg_sum + torch.sum(self.params[f'W{i+1}']**2)
        reg_sum = self.reg*reg_sum
        loss, dscores = softmax_loss(scores, y)
        loss = loss + reg_sum
        for i in range(self.num_layers-1, -1, -1):
          if i == (self.num_layers-1):
            dscores, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear.backward(dscores,
                                                                 cache_list.pop())
          else:
            if self.use_dropout:
              dscores = Dropout.backward(dscores, cache_list.pop())
            dscores, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear_ReLU.backward(dscores,
                                                                 cache_list.pop())
          grads[f'W{i+1}'] = grads[f'W{i+1}'] + 2*self.params[f'W{i+1}']*self.reg
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def get_three_layer_network_params():
    ...
    learning_rate = 8e-1  # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                             END OF YOUR CODE                 #
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - (config['learning_rate']*dw)/(grad_squared**0.5+config['epsilon'])
...
def adam(w, dw, config=None):
    ...
    moment1 = config['m']
    moment2 = config['v']
    moment1 = config['beta1']*moment1+(1-config['beta1'])*dw
    moment2 = config['beta2']*moment2+(1-config['beta2'])*dw*dw
    moment1_unbias = moment1/(1-config['beta1']**config['t'])
    moment2_unbias = moment2/(1-config['beta2']**config['t'])
    next_w = w-(config['learning_rate']*moment1_unbias)/(moment2_unbias**0.5+config['epsilon'])
    config['m'] = moment1
    config['v'] = moment2
    #########################################################################
    #                              END OF YOUR CODE                         #
