...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_stop = torch.arange(start, stop+1)
    test_vals = []
    for i in range(len(start_stop)):
        if start_stop[i] % 10 == 0:
            test_vals.append(start_stop[i])
        else:
            pass
    if len(test_vals) == 0:
        x = torch.empty((0,), dtype=torch.float64)
    else:
        x = torch.tensor(test_vals, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    row_size = len(x)
    col_size = max(x) + 1
    y = torch.zeros((row_size, col_size), dtype = torch.float32)
    #print(y)
    idx_row = range(row_size)
    #print(idx_row)
    idx_col = x
    #print(idx_col)
    y[idx_row, idx_col] = 1
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    mask = (x > 0)
    selected = x[mask].tolist()
    pos_sum = sum(selected)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    temp_1 = x.contiguous().view(2,3,4)
    #print(temp_1)
    temp_2 = temp_1.permute(1,0,2)
    #print(temp_2)
    y = temp_2.contiguous().view(3,8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    #print(temp)
    inside_sqrt = torch.sum(temp, dim=0) / (x.shape[0] - 1)
    #print(inside_sqrt)
    std = torch.sqrt(inside_sqrt)
    #print(std)
    y = (y - mean) / std
    ##########################################################################
    #                            END OF YOUR CODE                            #
